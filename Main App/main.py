    ...
    with st.sidebar:
        ...
        if not workout_started:
            ...
            if start_session_button:
                st.session_state.audio_to_play = TextToSpeech().speak("Workout started successfully")
            ...
            if end_session_button:
                try:
                    workout_time = int(time.time() - st.session_state.get("set_cycle_started_at", time.time()))
                    total_reps = int(st.session_state.get("reps", 0))
                    total_sets = int(st.session_state.get("sets_completed", 0))
                    exercise_name = st.session_state.get("exercise_type", "Unknown")
